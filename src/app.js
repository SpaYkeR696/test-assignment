...
import {format, getHours, addDays, isEqual, isAfter} from 'date-fns';
...
export default (dateNow) => {
  ...
  const presentDay = format(dateNow, 'dd/MM/yyyy');
  const presentHour = getHours(dateNow);
  const canReserve = (date, hour) => isAfter(date, presentDay)
  || (hour > presentHour && isEqual(date, presentDay));

  const initialTime = () => {
    if (presentHour >= 20) {
      const nextDay = addDays(presentDay, 1);
      state.time.currentDay = format(nextDay, 'dd/MM/yyyy');
